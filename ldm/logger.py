...
class ImageLogger:
    ...
    def log_img(self, model, batch, epoch,  input_dict, split="train", foldername=None, returngrid=False, warpeddepth=None, has_target=False, returnimg=False, savegrid=True, onlyretimg=False):

        model.eval()
        #model.train()
        # with torch.no_grad(): # TODO no_grad deactivated
        print("batch['image_target'] in log_img: ", batch['image_target'].shape)

        images = model.log_images(batch, input_dict, no_recon_cond=self.no_recon_cond, only_sample_output=self.only_sample_output, split=split, **self.log_images_kwargs)


        model.train()

        #ipdb.set_trace()
        print("Length images in log_img: ", len(images))
        if onlyretimg:
            ...
            # Counter percentage entries in imgs that are outside [-1,1]
            print("Log_img before clamp check")
            print("Shape images: ", imgs.shape)
            number_outside = torch.sum((imgs > 1) | (imgs < -1)).item()
            number_total = imgs.numel()
            print(f"Number points outside [-1,1]: {number_outside} out of {number_total}")
            print("Percentage of values outside [-1,1]: ", number_outside / number_total)
            # print("Percentage of values outside [-1,1]: ", torch.sum((imgs > 1) | (imgs < -1)).item() / imgs.numel())
